// Bảng "Lịch hẹn khám (check đặt lịch)" — bố cục MA TRẬN giống file Excel "Check
// đặt lịch" (feedback A2): mỗi NGÀY 1 khối; trong ngày các BÁC SĨ xếp CẠNH NHAU
// thành CỘT (cuộn ngang). Mỗi cột bác sĩ liệt kê lịch của họ: Khung giờ · Số
// khám · Thông tin BN · Phân loại khám. Read-only, data thật từ appointment.
// (queue_number/giờ ở DB phần lớn trống → hiện "—"/"Chưa có giờ", KHÔNG bịa.)
...
export default function WeeklyAppointmentsTable({ days }: { days: ApptDay[] }) {
  return (
    <>
      <div
        className={`${TBL_RESIZE} h-[480px] space-y-3 rounded-xl border border-[#f3cfe0] bg-[#fdf2f8] p-2`}
      >
        {days.map((day) => {
          ...
          return (
            ...
            >
              ...
              {day.items.length === 0 ? (
                ...
                // Các bác sĩ CẠNH NHAU thành cột (cuộn ngang như sheet Excel).
                <div className="flex gap-2 overflow-x-auto p-2">
                  {ordered.map(([doc, items]) => (
                    <div
                      key={doc}
                      className="flex min-w-[250px] flex-1 flex-col overflow-hidden rounded-lg border border-[#f6e0ec]"
                    >
                      <div className="flex items-center gap-2 bg-[#fdf5f9] px-3 py-1.5">
                        <span className="truncate text-xs font-semibold uppercase tracking-wide text-[#b83280]">
                      ...
                      <table className="w-full border-collapse text-sm">
                        <thead>
                          <tr className="bg-[#fce7f3] text-[11px] font-semibold uppercase tracking-wide text-[#9d2463]">
                            <th className="px-2 py-1.5 text-left">Giờ</th>
                            <th className="px-2 py-1.5 text-left">Số</th>
                            <th className="px-2 py-1.5 text-left">Thông tin BN</th>
                            <th className="px-2 py-1.5 text-left">Phân loại</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-[#f6e0ec]">
                          {items.map((a) => (
                            <tr
                              key={a.id}
                              className="align-top transition-colors hover:bg-[#fdf2f8]"
                            >
                              <td className="whitespace-nowrap px-2 py-1.5 text-[#171717]">
                                {fmtTimeOrNone(a.slot_start)}
                              </td>
                              <td className="whitespace-nowrap px-2 py-1.5 text-[#52525b]">
                                {a.queue_number ?? "—"}
                              </td>
                              <td className="px-2 py-1.5 text-[#171717]">
